...
│  ap-config/skills skills/  │   │  ApAgentWithAGUI → DeepSeek │
│  tasks/ toolbox.json chat/ │   └─────────────────────────────┘
└───────────────────────────┘
```
...
| Skill 目录    | `{仓库根}/.agents/ap-config/skills/`（内置）与 `{仓库根}/.agents/skills/` |

#### 两套路径解析策略

1. **`getApWebAgentsRoot()`** — 基于 `INIT_CWD`（用户敲 `pnpm ap-web:dev` 时的目录），用于任务、toolbox、文件监听
2. **`getWorkspaceRoot()`** — 向上查找含 `pnpm-workspace.yaml`、`.agents/ap-config` 或 `.agents/skills` 的目录，用于 Agent 的 `cwd`、Skill 列表、AI 配置
...
| `GET /api/skills`               | 列出 ap-config/skills 与 .agents/skills |
...
- `ToolsColumn` 从 `.agents/ap-config/skills` 与 `.agents/skills` 选 Skill 加入 toolbox
