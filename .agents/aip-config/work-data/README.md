...
本目录是 work-data **种子**。`pnpm install` / `pnpm aip` 启动时拷贝到 `.agents/aip-config/work-data`；目标中已存在的同名文件不覆盖。AI 只处理 `.agents/aip-config/work-data`。

```text
.agents/aip-config/work-data/
├── README.md
├── tasks/
...
- 执行流程见 `aip/src/skills/aip-task-executor/SKILL.md`
