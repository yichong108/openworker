...
/**
 * 按调用名解析 skill：精确目录名，或 `aip-` 前缀的短名。
 *
 * 例如目录 `aip-task-executor` 可用 `aip-task-executor` 或 `task-executor`。
 * 若同时存在短名目录与 `aip-<短名>`，精确匹配优先。
 *
...
export function resolveAgentsSkill(
  ...
  if (name.startsWith('aip-')) return undefined
  return skills.find((skill) => skill.name === `aip-${name}`)
}

/**
 * 按名称解析 `.agents/skills/<name>`，支持省略 `aip-` 前缀。
 *
 * @param workspaceRoot - 仓库根目录
...
/**
 * `aip-` 开头 skill 的短名（去掉前缀）；其它 skill 返回 undefined。
 *
 * @param skillName - 目录名
 * @returns 短名；没有 `aip-` 前缀则为 undefined
 */
export function skillShortName(skillName: string): string | undefined {
  if (!skillName.startsWith('aip-')) return undefined
  const short = skillName.slice('aip-'.length)
