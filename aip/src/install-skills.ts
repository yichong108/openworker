/**
 * aip 初始化：覆盖安装同名 skill，并把 work-data 种子补齐到 .agents/aip-config/work-data。
...
/**
 * 用 `aip/src/skills` 下的各个 skill 覆盖仓库根 `.agents/skills` 中的同名目录。
 *
 ...
 */
export function installAipSkills(): string {
  ...
  process.stderr.write(`[aip] 已覆盖安装 skill ${skillNames.join(', ') || '(无)'} → ${destRoot}\n`)
  return destRoot
}

/**
 * 把 `aip/src/work-data` 补齐到仓库根 `.agents/aip-config/work-data`。
 *
 * 只拷贝目标中尚不存在的文件；同名文件不覆盖，也不删除目标里多出来的内容。
 *
 * @returns 安装目标路径（`.agents/aip-config/work-data`）
 */
export function installAipConfig(): string {
  const packageRoot = getPackageRoot()
  const workspaceRoot = findWorkspaceRoot(packageRoot)
  const source = join(packageRoot, 'src', 'work-data')
  const dest = join(workspaceRoot, '.agents', 'aip-config', 'work-data')
  ...
  process.stderr.write(
    `[aip] 已补齐 aip-config/work-data（新增 ${copied}，跳过 ${skipped} 个同名文件） → ${dest}\n`
  )
  return dest
}

/**
 * 执行 aip 工作区初始化：覆盖同名 skill，并补齐 aip-config/work-data。
 */
export function installAipWorkspace(): void {
  installAipSkills()
  installAipConfig()
}
