 ...
 *
 * 从 `aip/` 子包启动时 cwd 可能不是仓库根，Agent 必须在仓库根上跑才能看到
 * `aip/src/skills` 与 `.agents/aip-config/work-data/tasks`。
 *
 * @param startDir - 起始目录
 ...
 */
export function loadAipEnv(): void {
...
/** Cursor SDK 对话模式 */
export type AipConversationMode = 'agent' | 'plan'

/** SDK 对话模式默认值：直接改代码 */
export const DEFAULT_AIP_MODE: AipConversationMode = 'agent'
 ...
 */
export function parseConversationMode(value: string): AipConversationMode {
 ...
 * 解析本次运行使用的 SDK 对话模式。
 *
 * @returns AIP_MODE 或默认 agent
 */
export function resolveConversationMode(): AipConversationMode {
  const raw = process.env.AIP_MODE?.trim()
  if (!raw) return DEFAULT_AIP_MODE
