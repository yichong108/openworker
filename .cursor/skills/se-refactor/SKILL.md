...
description: >
  ...
  或 .ai/tasks/ 中存在重构类任务时使用。
  纯功能实现、按需求开发任务应使用 se-task-executor，不要用本技能代替。
license: MIT
metadata:
  version: '0.1.0'
  author: Wang Zhang
...
项目级规则由 `AGENTS.md` 定义。架构决策由 `.ai/decisions/` 中的 ADR 定义。

不要把重构做成新功能、不要顺手修无关 bug、不要自行发明任务中未提出的需求。

---

## 与 se-task-executor 的区别

|      | se-task-executor     | se-refactor                    |
| ---- | -------------------- | ------------------------------ |
...
5. **服从 ADR。** 只读 `.ai/decisions/`。需要新架构决策时提出方案并停止，不得擅自创建或修改 ADR。
...
2. 确定入口：`.ai/tasks/active/` 中的重构任务，或用户指定的代码范围。
...
1. 只做一种变换。
2. 更新调用点与导出，保持编译通过。
3. 导出函数按 `AGENTS.md` 补注释（用途、意图、为何如此实现）。
...
- 若有 TASK：把 Status 设为 `COMPLETED`，填写 Completion，再移到 `.ai/tasks/archive/<年份>/`。
...
处理 `.ai/tasks/active/` 时：

- 只领取目标为重构/结构调整、且依赖已满足的任务。
- 功能实现类任务留给 `se-task-executor`。
