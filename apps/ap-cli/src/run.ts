...
async function runCursorPrompt(input: {
  ...
  process.stderr.write(
    `[ap] name=${input.name} agent=${agent.agentId} cwd=${input.cwd} model=${input.model} mode=${input.mode}\n`
  )
  ...
  try {
    ...
    process.stderr.write(`[ap] run=${run.id}\n`)
    ...
    if (result.usage) {
      process.stderr.write(
        `\n[ap] tokens total=${result.usage.totalTokens} in=${result.usage.inputTokens} out=${result.usage.outputTokens}\n`
      )
    }

    if (result.status === 'finished') return 0
    if (result.status === 'cancelled') return 130
    process.stderr.write(`[ap] run failed: ${result.id} ${result.error?.message ?? ''}\n`)
    return 2
  } catch (err) {
    if (err instanceof CursorAgentError) {
      process.stderr.write(`[ap] startup failed: ${err.message} retryable=${err.isRetryable}\n`)
      return 1
    }
    throw err
  } finally {
