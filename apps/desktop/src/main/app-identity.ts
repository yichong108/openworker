 ...
 * 身份字段来自 `env-bootstrap` 已加载的 `.env-*`（开发）或 `openworker.env`（安装包）。
 */
...
if (!productName) {
  throw new Error('未设置 OPENWORKER_PRODUCT_NAME，请检查渠道环境文件')
}
if (!appUserModelId) {
  throw new Error('未设置 OPENWORKER_APP_USER_MODEL_ID，请检查渠道环境文件')
}
